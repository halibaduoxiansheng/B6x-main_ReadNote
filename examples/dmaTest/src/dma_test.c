...
/*data length -> stop data legnth -> parity bit*/
...
static void dmaUartLoop(void)
{
    ...
    if (txChnlBusy) /*avoid repeat use DMA transmit*/
    {
        return;
    }
    ...
    /**
     * UART read
     * DMA Transmit to UART TRANS Register
     */
...
void dmaTest(void)
{
    ...
    // config RX chnl
    /*double buffer mode, impove efficiency*/
    DMA_UARTx_RX_CONF(DMA_CH_UART_RX, 1, &rxdBuffer[0], RXD_BUFF_HALF, CCM_PING_PONG); /*upper half area*/
    DMA_UARTx_RX_CONF(DMA_CH_UART_RX | DMA_CH_ALT, 1, &rxdBuffer[RXD_BUFF_HALF], RXD_BUFF_HALF, CCM_PING_PONG); /*lower half area*/
    #else
    // init dma chnl
    ...
    /*tran rate 115200 Bytes each seconds*/
    uart_conf(TEST_PORT, TEST_BAUD, TEST_LCRS); /* LCRS -> Line Control Register*/
    #endif

    uart_fctl(TEST_PORT, TEST_FCTL, TEST_RTOR, TEST_INTR); // RTO_EN (Revice TimeOut Enable)
    uart_mctl(TEST_PORT, 1); // DMA_EN (DMA trans Enable)
